...
(function (global) {
  ...
  function dashHost() {
    ...
    if (sub) return 'https://' + sub + '.biz1.co.il';
    return DOMAIN;
  }
  ...
  async function apiSaveProject(payload) {
    ...
    if (id && id !== '0') {
      try {
        raw = await tryRequest(['Projects.Update', 'Project.Update'], Object.assign({ id: id }, payload));
      } catch (err) {
        raw = await tryRequest(['Projects.Add', 'Projects.Save', 'Project.Save'], payload);
      }
    } else {
      raw = await tryRequest(['Projects.Add', 'Projects.Save', 'Project.Save'], payload);
    }
  ...
  async function apiLoadBoard(payload) {
    ...
    var columns = (colRows.length ? colRows : [
      ...
      return {
        key: key,
        label: col.name_en || col.name_he || col.label || col.column_name || key,
    ...
    var teamSrc = project.team_members || project.users || project.members || project.team || project.assignees || [];
  ...
  async function apiCreateMission(payload) {
    ...
    var body = {
      ...
      data_mission_type: payload.data_mission_type || payload.project_column || 'to_do'
    };
...
(function (global) {
  ...
  function clientHtml(p) {
    ...
    if (!name) return '<span class="assign-hint">' + esc(tr('no_client')) + '</span>';
  ...
  function renderBoard() {
    ...
    if (clientEl) {
      var boardClient = textOf(board.client_name) || textOf(board.client) || textOf(board.customer);
      if (boardClient) {
        clientEl.innerHTML = '<span class="avatar" style="background:' + colorFor(boardClient) + '">' +
          esc(initials(boardClient)) + '</span><span>' + esc(boardClient) + '</span>';
      } else {
  ...
  function initLogin() {
    ...
    document.querySelectorAll('.demo-user-btn').forEach(function (btn) {
      ...
      btn.addEventListener('click', function (event) {
        ...
        if (usernameEl) usernameEl.value = btn.getAttribute('data-user') || 'dhara@yopmail.com';
        if (passwordEl) passwordEl.value = btn.getAttribute('data-pass') || 'pass0364';
        clearError();
        runLogin();
      });
    });
