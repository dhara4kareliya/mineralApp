window.Biz1Api = (function () {
  function resolveDomain() {
    ...
    if (cfg.API_DOMAIN) return String(cfg.API_DOMAIN).replace(/\/+$/, '');
    var user = String(cfg.user || 'eli')
      .trim()
      .toLowerCase()
      .replace(/^https?:\/\//, '')
      .replace(/\.biz1\.co\.il.*$/i, '')
      .replace(/\.bull36\.com.*$/i, '')
      .replace(/[^a-z0-9-]/g, '');
    return 'https://' + (user || 'eli') + '.biz1.co.il';
  }

  const API_DOMAIN = resolveDomain();
  const state = {
    client: null,
    domain: API_DOMAIN,
  ...
  async function ensureSdk() {
    ...
    // Never load remote SDK / Socket.IO — CSP script-src 'self' blocks it.
    // biz1-sdk.js must be included as a static script tag.
    if (!window.Biz1SDK) {
      throw new Error('Biz1 SDK missing. Deploy assets/biz1-sdk.js with the app.');
    }
  }
  ...
  async function login(credentials) {
    ...
    state.basic = basic;
    state.user =
      (basic && (basic.user || (basic.data && basic.data.user))) ||
      (auth.raw && auth.raw.user) ||
      null;
    try {
  ...
  async function connectRealtime(onEvent, onReadyState) {
    const client = await ensureClient();
    if (!window.io) {
      if (onReadyState) onReadyState(false, null);
      return;
    }
    try {
