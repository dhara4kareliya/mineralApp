...
const Api = (function () {
  ...
  function loadScript(src, timeoutMs) {
    return new Promise((resolve, reject) => {
      ...
      if (existing) {
        // Already in the page (static <script> tag) — do not remove/reload.
  ...
  async function ensureSdk(domain) {
    ...
    // Never load remote /app/sdk/biz1-sdk.js — CSP script-src 'self' blocks it.
    if (window.Biz1SDK) {
      sdkLoaded = true;
    } else if (!sdkLoaded) {
  ...
  async function uploadFile({ customerId, file, fileName, folder, subFolder }) {
    ...
    // https://eli.biz1.co.il/app/help/Files.Upload
