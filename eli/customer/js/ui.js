...
/** CSP-safe navigation for rows using data-nav-href / data-nav-open (no inline onclick). */
document.addEventListener('click', (e) => {
  const openEl = e.target.closest('[data-nav-open]');
  if (openEl) {
    const url = openEl.getAttribute('data-nav-open');
    if (url) window.open(url, '_blank', 'noopener');
    return;
  }
  const hrefEl = e.target.closest('[data-nav-href]');
  if (hrefEl) {
    if (e.target.closest('a,button,[data-stop]')) return;
    const href = hrefEl.getAttribute('data-nav-href');
