          ...
          (w) => `<div class="card" ${w.demo ? '' : 'data-live-target'}>
            <div class="card-header">
              ...
              <div class="flex gap-1">
                ${w.demo ? `<button type="button" class="btn btn-sm btn-ghost" id="btn-demo-live">${I18n.t('dash.pulseDemo')}</button>` : ''}
                ${w.link ? `<a class="btn btn-sm btn-secondary" href="${w.link}">${I18n.t('viewAll')}</a>` : ''}
              </div>
            </div>
            <div id="${w.id}">${w.html ? w.html : '<div class="spinner"></div>'}</div>
          </div>`
        )
        .join('');

      document.getElementById('btn-demo-live')?.addEventListener('click', () => {
        Realtime.simulate('whatsapp');
      });

      widgetDefs.forEach((w) => {
        if (w.html || !w.result) return;
        try {
          renderList(w.id, w.result, w.render, w.head, w.empty);
        } catch (err) {
          const el = document.getElementById(w.id);
          if (el) Layout.error(el, err.message || 'Failed to render');
        }
      });
    })();

    function renderList(id, result, rowFn, head, emptyMsg) {
      const el = document.getElementById(id);
      if (!el) return;
      if (!result.ok) {
        el.innerHTML = `<div class="alert alert-error">${Layout.escapeHtml(result.error?.message || I18n.t('failedToLoad'))}</div>`;
        return;
      }
      const rows = UI.listRows(result.data);
      if (!rows.length) {
        Layout.empty(el, emptyMsg, I18n.t('dataWillAppear'));
        return;
      }
      el.innerHTML = `<div class="table-wrap"><table><thead><tr>${head}</tr></thead><tbody>${rows.map(rowFn).join('')}</tbody></table></div>`;
    }

    Realtime.on((payload) => {
      if (payload.type === 'ticket' || payload.type === 'whatsapp') {
        // soft refresh signal
      }
    });
