(function () {
  ...
  function paginationParams() {
    ...
    return { limit: PAGE_SIZE, length: PAGE_SIZE, start: start, offset: start };
  }
  ...
  function renderPagination() {
    ...
    var show = hasRows && pages > 1;
    paginationEl.classList.toggle('hidden', !show);
    if (!show) return;

    var from = (currentPage - 1) * PAGE_SIZE + 1;
    var to = Math.min(currentPage * PAGE_SIZE, totalCount);
    paginationInfo.textContent = I18n.t('pageInfo', { from: from, to: to, total: totalCount });
  ...
  async function loadList() {
    ...
    try {
      ...
      var list = await api.listExpenses(Object.assign({}, filters, paginationParams()));
      var count = await api.countExpenses(filters);
      if (seq !== listLoadSeq) return;

      totalCount = count || list.total || 0;
      var pages = totalPages();
      ...
      totalLabel.textContent = I18n.t('total') + ': ' + totalCount;
      expenseList.innerHTML = '';
  ...
  function expenseFileUrls(raw) {
    return String(raw || '')
      .split('----@----')
      .map(function (u) {
        return ExpenseApp.resolveMediaUrl(u);
      })
      .filter(Boolean);
  }
  ...
  function isImageFileUrl(url) {
    var text = String(url || '');
    if (/\.(pdf|doc|docx|xls|xlsx|zip|csv|txt|mp4|mov|avi)(\?|$)/i.test(text)) return false;
    return /\.(png|jpe?g|gif|webp|bmp|svg)(\?|$)/i.test(text) || /\/upload\/[^?]+\.(png|jpe?g|gif|webp|bmp|svg)(\?|$)/i.test(text);
  }
  ...
  function fileNameFromUrl(url) {
    try {
      ...
      return decodeURIComponent(name) || I18n.t('attachment');
    } catch (e) {
  ...
  async function openDetail(id) {
    ...
    try {
      ...
      var imageUrls = expenseFileUrls(expense.image || expense.file_path || expense.file || expense.thumbnail || '');
      ...
      if (imageUrls.length) {
        var hasImage = imageUrls.some(isImageFileUrl);
        html +=
          '<div class="field" style="margin-top:16px;margin-bottom:8px;"><label>' +
          I18n.t(hasImage && imageUrls.length === 1 ? 'image' : imageUrls.length > 1 ? 'attachments' : 'attachment') +
          (imageUrls.length > 1 ? ' (' + imageUrls.length + ')' : '') +
          '</label><div class="detail-images">';
        imageUrls.forEach(function (imageUrl, index) {
          var isImg = isImageFileUrl(imageUrl);
          var fileName = fileNameFromUrl(imageUrl);
          html +=
            '<a class="detail-image card' +
            (isImg ? '' : ' detail-file') +
            '" href="' +
            imageUrl +
          ...
          if (isImg) {
            ...
            html +=
              '<div class="detail-file-row"><span class="detail-file-name"></span><span class="detail-file-action">' +
              I18n.t('openFile') +
              '</span></div>';
          }
          html += '</a>';
      ...
      detailBody.querySelectorAll('.detail-file-name').forEach(function (el, idx) {
        var url = imageUrls.filter(function (u) {
          return !isImageFileUrl(u);
        })[idx];
        el.textContent = url ? fileNameFromUrl(url) : I18n.t('attachment');
      });
      detailBody.querySelectorAll('.detail-image img').forEach(function (img) {
        img.addEventListener('error', function () {
          var wrap = img.closest('.detail-image');
          if (!wrap || wrap.classList.contains('detail-file')) return;
          wrap.classList.add('detail-file');
          wrap.innerHTML =
            '<div class="detail-file-row"><span class="detail-file-name"></span><span class="detail-file-action">' +
            I18n.t('openFile') +
            '</span></div>';
          var nameEl = wrap.querySelector('.detail-file-name');
          if (nameEl) nameEl.textContent = fileNameFromUrl(wrap.getAttribute('href') || img.getAttribute('src') || '');
        });
      });
