(function (global) {
  ...
  ExpenseApi.prototype.listExpenses = async function (filters) {
    var opts = Object.assign({ limit: 25, start: 0 }, filters || {});
    if (opts.offset == null) opts.offset = opts.start;
    var raw = await this.request('Expenses.List', opts);
  ...
  function resolveMediaUrl(value) {
    ...
    if (/^biz1upload\//i.test(cleaned) || /\/upload\//i.test(cleaned) || /\.(png|jpe?g|gif|webp|bmp|svg|pdf)(\?|$)/i.test(cleaned)) {
      return 'https://files.biz1.co.il/' + cleaned;
    }
    var domain = apiDomain();
    if (domain) return domain.replace(/\/+$/, '') + '/' + cleaned;
    return text;
  }
