...
(function (global) {
  ...
  function rememberDashboardOrigin(url) {
    try {
      var origin = typeof url === 'string' && /^https?:\/\//i.test(url)
        ...
        : ('https://' + String(url || '').toLowerCase()
            .replace(/\.biz1\.co\.il.*$/i, '')
            .replace(/\.bull36\.com.*$/i, '') + '.biz1.co.il');
      if (origin && (origin.indexOf('biz1.co.il') !== -1 || origin.indexOf('bull36.com') !== -1)) {
  ...
  async function approveOrderProposal(documentId) {
    ...
    // Prefer Biz1 app API (no PHP proxy — ZIP hosts block .php).
    var apiAttempts = [
      { route: 'Documents.Edit', body: { document_id: docId, id: docId, approve: 1 } },
      { route: 'Documents.Update', body: { document_id: docId, id: docId, approve: 1 } },
      { route: 'Documents.Approve', body: { document_id: docId, id: docId, order_proposals_id: docId } }
    ];
    for (var a = 0; a < apiAttempts.length; a++) {
      try {
        var raw = await client.request(apiAttempts[a].route, apiAttempts[a].body);
        if (raw && (Number(raw.success) === 1 || raw.success === true || raw.ok === true || !raw.message)) {
          return { ok: true, raw: raw, route: apiAttempts[a].route };
        }
        lastErr = new Error((raw && raw.message) || (apiAttempts[a].route + ' failed'));
        lastErr.raw = raw;
      } catch (errApi) {
        lastErr = errApi;
      }
    ...
    // Fallback: dashboard cookie flow (same-origin / environments that expose Set-Cookie).
    try {
  ...
  async function submitSignature(payload) {
    ...
    if (!crmApprove || crmApprove.ok === false) {
      var fail = new Error(
        ...
        'CRM status was not updated to Approved. Try again or approve the proposal in Biz1.'
      );
      fail.raw = {
        upload: upload,
