...
(function () {
  ...
  function renderCustomer(c, kind) {
    ...
    var chatUrl = 'צ_אט עם לקוח.dc.html?customer_id=' + encodeURIComponent(id) +
      '&name=' + encodeURIComponent(name) +
      (phone ? '&phone=' + encodeURIComponent(phone) : '');
    var missionUrl = 'שירות צור משימה.dc.html?customer_id=' + encodeURIComponent(id) +
      '&name=' + encodeURIComponent(name) +
      (phone ? '&phone=' + encodeURIComponent(phone) : '') +
  ...
  async function loadCustomer(mount) {
    ...
    if (!customerId) {
      var back = kind === 'customer' ? 'לקוחות.dc.html' : 'רשימת לידים.dc.html';
      el.innerHTML =
