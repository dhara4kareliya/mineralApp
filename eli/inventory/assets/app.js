(function () {
  ...
  async function startScanner() {
    stopScanner();
    var container = document.getElementById('qrReader');
    if (!container) return;
    container.innerHTML = '';
    ...
    try {
      ...
      var formats = ['ean_13', 'ean_8', 'code_128', 'code_39', 'upc_a', 'upc_e', 'qr_code', 'codabar'];
      try {
        state.barcodeDetector = new BarcodeDetector({ formats: formats });
      } catch (e) {
        state.barcodeDetector = new BarcodeDetector();
      }

      state.scanTimer = setInterval(function () {
        if (!state.scanning || state.scanPaused || !state.barcodeDetector) return;
        if (video.readyState < 2) return;
        state.barcodeDetector.detect(video).then(function (codes) {
          if (!codes || !codes.length || state.scanPaused) return;
          var value = String(codes[0].rawValue || '').trim();
          if (!value) return;
          state.scanPaused = true;
          lookupBarcode(value);
        }).catch(function () { /* ignore frame miss */ });
      }, 450);
    } catch (err) {
  ...
  function closeStockModal() {
    ...
    if (!els.scanner.classList.contains('hidden') && state.scanning) {
      state.scanPaused = false;
    }
  }
  ...
  function buildPoPdfHtml() {
    ...
    return '' +
      ...
      'font-family:system-ui,-apple-system,\'Segoe UI\',Arial,sans-serif;box-sizing:border-box;">' +
  ...
  function generatePdf() {
    ...
    var filename = 'PO-Biz1-' + Date.now() + '.html';
    var dir = state.lang === 'he' ? 'rtl' : 'ltr';
    var html =
      '<!DOCTYPE html><html lang="' + (state.lang || 'en') + '" dir="' + dir + '"><head>' +
      '<meta charset="utf-8"><title>' + filename + '</title>' +
      '<style>body{margin:24px;font-family:system-ui,-apple-system,Segoe UI,Arial,sans-serif;color:#111}' +
      '@media print{body{margin:0}}@media screen{body::after{content:"Use Print → Save as PDF";display:block;margin-top:16px;color:#666;font-size:12px}}</style></head><body>' +
      buildPoPdfHtml() +
      '</body></html>';
