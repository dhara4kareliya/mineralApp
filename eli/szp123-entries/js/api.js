 ...
 * Docs: https://eli.biz1.co.il/app/help/category/Entries
 */
(function (global) {
  'use strict';
  ...
  function accountUsername(basic) {
    ...
    if (forced && isAccountHandle(forced)) return String(forced).trim();
    var fromBasic = accountUsernameFromBasic(basic || loadUserBasic());
    if (fromBasic) return fromBasic;
    var pathUser = cfg().PATH_USERNAME;
    if (pathUser && isAccountHandle(pathUser)) return String(pathUser).trim();
    var pathStored = '';
    ...
    if (isAccountHandle(sub) && !/^(www|apps|user)$/i.test(sub)) return sub;
    return 'eli';
  }

  function apiRootDomain() {
    var root = String(cfg().API_ROOT || '').replace(/^\.+/, '');
    if (root) return root;
    var host = String((global.location && location.hostname) || '');
    if (host.indexOf('bull36.com') >= 0) return 'bull36.com';
    return 'biz1.co.il';
  }

  function apiDomain() {
    var fixed = String(cfg().API_DOMAIN || '').replace(/\/+$/, '');
    if (fixed) return fixed;
    var user = accountUsername() || 'eli';
    return ('https://' + user + '.' + apiRootDomain()).replace(/\/+$/, '');
  }

  function appsHost() {
    try {
      if (isAppsHost()) return String(location.protocol + '//' + location.host).replace(/\/+$/, '');
    } catch (e) { /* ignore */ }
    var host = String((global.location && location.hostname) || '');
    if (host.indexOf('bull36.com') >= 0) return 'https://apps.bull36.com';
    return String(cfg().APPS_HOST || 'https://apps.biz1.co.il').replace(/\/+$/, '');
  }
  ...
  function isAppsHost() {
    var host = String((global.location && location.hostname) || '');
    return host.indexOf('apps.biz1.co.il') >= 0 || host.indexOf('apps.bull36.com') >= 0;
  }

  function appsPathUser() {
    var pathUser = cfg().PATH_USERNAME;
    if (pathUser && isAccountHandle(pathUser)) return String(pathUser).trim();
    var parts = String((global.location && location.pathname) || '').split('/').filter(Boolean);
    if (parts.length >= 1 && isAccountHandle(parts[0]) && !/^(css|js|login)$/i.test(parts[0])) {
      return parts[0];
    }
    try {
      var stored = sessionStorage.getItem('entries_biz1_path_user') || '';
      if (isAccountHandle(stored) && !/^(entries|login|css|js)$/i.test(stored)) return stored;
    } catch (e) { /* ignore */ }
    return accountUsername() || 'eli';
  }

  /** Current deploy folder, e.g. https://apps.bull36.com/eli/szp123-entries/ */
  function appDeployBase() {
    try {
      var configured = String(cfg().APP_BASE_PATH || '').trim();
      if (configured && configured.charAt(0) === '/') {
        return appsHost() + configured.replace(/\/?$/, '/');
      }
      var parts = String(location.pathname || '').split('/').filter(Boolean);
      if (isAppsHost() && parts.length >= 2) {
        return appsHost() + '/' + encodeURIComponent(parts[0]) + '/' + encodeURIComponent(parts[1]) + '/';
      }
  ...
  function appHomeUrl(username) {
    // Always stay on this deployment URL — never rewrite /eli/szp123-entries/ → /eli/entries/
    void username;
    try {
      var parts = String(location.pathname || '').split('/').filter(Boolean);
      if (parts.length >= 2) {
        return String(location.origin) + '/' + parts[0] + '/' + parts[1] + '/';
      }
      return new URL('./', location.href).href;
    } catch (e) {
      return './';
  ...
  function ensureAccountUrl(username) {
    // Never redirect. Hosted path is the source of truth (e.g. /eli/szp123-entries/).
    var user = String(username || appsPathUser() || accountUsername() || '').trim();
    if (user) rememberPathUsername(user);
    return false;
  }
