...
(function () {
  ...
  function endFolderStripScroll(e) {
    if (!folderStripScroll.active) return;
    if (e && folderStripScroll.pointerId != null && e.pointerId !== folderStripScroll.pointerId) return;
    folderStripScroll.active = false;
    if (folderStrip) {
      folderStrip.classList.remove('is-dragging');
      if (folderStripScroll.pointerId != null && folderStrip.releasePointerCapture) {
        try { folderStrip.releasePointerCapture(folderStripScroll.pointerId); } catch (err) { /* ignore */ }
      }
    }
    folderStripScroll.pointerId = null;
  }
  ...
  if (folderStrip) {
    folderStrip.addEventListener('pointerdown', function (e) {
      if (e.pointerType === 'mouse' && e.button !== 0) return;
      folderStripScroll.active = true;
      folderStripScroll.moved = false;
      folderStripScroll.pointerId = e.pointerId;
      folderStripScroll.startX = e.clientX;
      folderStripScroll.startScroll = folderStrip.scrollLeft;
    });
    folderStrip.addEventListener('pointermove', function (e) {
      if (!folderStripScroll.active || e.pointerId !== folderStripScroll.pointerId) return;
      var dx = e.clientX - folderStripScroll.startX;
      if (!folderStripScroll.moved && Math.abs(dx) < 6) return;
      if (!folderStripScroll.moved) {
        folderStripScroll.moved = true;
        folderStrip.classList.add('is-dragging');
        try { folderStrip.setPointerCapture(e.pointerId); } catch (err) { /* ignore */ }
      }
      folderStrip.scrollLeft = folderStripScroll.startScroll - dx;
      e.preventDefault();
    });
    folderStrip.addEventListener('pointerup', endFolderStripScroll);
    folderStrip.addEventListener('pointercancel', endFolderStripScroll);
    folderStrip.addEventListener('click', function (e) {
      ...
      if (!chip || !folderStrip.contains(chip)) return;
