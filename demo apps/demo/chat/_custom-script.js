	$(function() {
	/*==JQUERY SELECTBOX==*/
	$("body .chzn-select").chosen(); 
	$("body .chzn-select-deselect").chosen({allow_single_deselect: true});
	/*==JQUERY UNIFORM==*/
	$(".checkbox,.rem_me,.radio,input[type='file']:not([not_uniform='true'])").uniform();
	...
	$(".switch_bar ul,#primary_nav ul").sortable();
...
$('body .action-icons').tipsy({
	title: 'title',
	fade: 'out',     // fade tooltips in/out?
    gravity: 's', });// nw | n | ne | w | e | sw | s | se

$('body .post_action_bar a, body .invoice_action_bar a').tipsy({
	title: 'title',
	fade: 'out',     // fade tooltips in/out?
    gravity: 's', });// nw | n | ne | w | e | sw | s | se
...
function confirm(message, callback) {
	$('#confirm').modal({
		closeHTML: "<a href='javascript:;' title='Close' class='modal-close'>x</a>",
		position: ["20%",],
...
$(function() {
	...
	var image_url = window.base_url+"new/images/details_open.png";
    nCloneTd.innerHTML = '<img src='+image_url+'>';
    ...
    $('.tbl_details tbody td img').live('click', function () {
        ...
        if ( oTable.fnIsOpen(nTr) )
        {
            ...
            this.src = window.base_url+"new/images/details_open.png";
            oTable.fnClose( nTr );
        }
        else
        {
            /* Open this row */
            this.src = window.base_url+"new/images/details_close.png";
