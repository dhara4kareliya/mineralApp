...
(function (global) {
  ...
  var SCREEN_API = {
    ...
    'גבייה': { routes: ['PaymentForms.*', 'Settings.SaveCard'], status: 'live' }
  };
    ...
    try {
      ...
      try {
        ...
        for (var i = 0; i < global.localStorage.length; i++) {
          ...
          if (k && k.indexOf('biz1_payment_gateways_cache') === 0) keys.push(k);
        }
  ...
  async function login(opts) {
    ...
    try {
      connectRealtime().catch(function () { /* optional until home wires UI */ });
    } catch (eSock) { /* ignore */ }
  ...
  function extractListRows(raw) {
    if (!raw) return [];
    if (Array.isArray(raw)) return raw;
    if (Array.isArray(raw.data)) return raw.data;
    if (Array.isArray(raw.rows)) return raw.rows;
    if (Array.isArray(raw.forms)) return raw.forms;
  ...
  function normalizePaymentFormRow(row) {
    if (!row || typeof row !== 'object') return null;
    var id = row.id != null ? row.id : (row.payment_form_id != null ? row.payment_form_id : row.data_id);
    var link = row.template_payment_link ||
      row.payment_form_link ||
      row.payment_link ||
      row.public_link ||
      row.data_copy ||
      '';
    return {
      ...
      invoice_setting_id: row.invoice_setting_id != null ? row.invoice_setting_id : '',
      payment_gatway: row.payment_gatway != null ? row.payment_gatway : (row.payment_gateway != null ? row.payment_gateway : ''),
      iteam_id: row.iteam_id != null ? row.iteam_id : (row.item_id != null ? row.item_id : (row.payment_item_id != null ? row.payment_item_id : '')),
      document_type: row.document_type || '',
      ask_to_customer: row.ask_to_customer,
      change_value_with_customer: row.change_value_with_customer,
      required_fields: row.required_fields,
      payment_type_data: row.payment_type_data,
      template_payment_link: String(link || '').trim(),
      item_name: row.item_name || '',
      allow_wild_card_product: row.allow_wild_card_product,
      need_to_show_company_id: row.need_to_show_company_id,
      raw: row
    };
  }

  /** Public customer pay URL (no app login). Live keeps ?hased_id={hased_id}. */
  function formatPublicPaymentLink(link) {
    var url = String(link == null ? '' : link).trim();
    if (!url) return '';
    return url;
  }

  async function resolvePaymentFormShareLink(paymentFormIdOrRow) {
    var row = paymentFormIdOrRow;
    if (row && typeof row === 'object') {
      var existing = formatPublicPaymentLink(row.template_payment_link);
      if (existing) return existing;
      var id = row.id != null ? row.id : row.payment_form_id;
      if (id == null) return '';
      var detail = await getPaymentForm(id);
      return formatPublicPaymentLink(detail.form && detail.form.template_payment_link);
    }
  ...
  function extractPaymentFormRows(raw) {
    return extractListRows(raw).map(normalizePaymentFormRow).filter(Boolean);
  }

  function extractCount(raw) {
    if (!raw || typeof raw !== 'object') return 0;
    var n = raw.count != null ? Number(raw.count)
      : (raw.total_record != null ? Number(raw.total_record)
        : (raw.recordsFiltered != null ? Number(raw.recordsFiltered)
          : (raw.recordsTotal != null ? Number(raw.recordsTotal) : 0)));
  ...
  /** PaymentForms.List — org payment form templates (≤25). */
  async function listPaymentForms(extra) {
    ...
    if (p.search) {
      ...
      payload.search_text = p.search;
      payload.filter_data = p.search;
      payload.form_name = p.search;
    }
    if (p.item_type || p.iteam_type) {
      payload.iteam_type = p.iteam_type || p.item_type;
      payload.item_type = payload.iteam_type;
    }
    var raw = await client.request('PaymentForms.List', payload);
    var rows = extractPaymentFormRows(raw);
    var count = extractCount(raw);
    if (!count) count = rows.length;
    return { raw: raw, rows: rows, count: count };
  }

  /** PaymentForms.Count — same filters as List. */
  async function countPaymentForms(extra) {
    var client = getClient();
    var p = extra || {};
    var payload = Object.assign({}, p);
    if (p.search) payload.search = p.search;
    if (p.item_type || p.iteam_type) payload.iteam_type = p.iteam_type || p.item_type;
    var raw = await client.request('PaymentForms.Count', payload);
    return { count: extractCount(raw), raw: raw };
  }

  /** PaymentForms.Get — one template by id. */
  async function getPaymentForm(paymentFormId, extra) {
    var id = requireId(paymentFormId, 'payment_form_id');
    var client = getClient();
    var raw = await client.request('PaymentForms.Get', Object.assign({
      payment_form_id: id,
      id: id,
      data_id: id
    }, extra || {}));
    assertApiSuccess(raw, 'PaymentForms.Get', 'Payment form not found.');
    var data = (raw && raw.data && typeof raw.data === 'object' && !Array.isArray(raw.data))
      ? raw.data
      : (raw && raw.output && typeof raw.output === 'object' ? raw.output : raw);
    var form = normalizePaymentFormRow(data);
    if (!form || form.id == null) {
      var err = new Error((raw && raw.message) || 'Payment form not found.');
      err.route = 'PaymentForms.Get';
      err.raw = raw;
      throw err;
    }
    return { form: form, raw: raw };
  }

  /** PaymentForms.Add — create template. */
  async function addPaymentForm(params) {
    var p = params || {};
    var formName = String(p.form_name || p.name || '').trim();
    if (!formName) {
      var e = new Error('Missing required parameter: form_name');
      e.route = 'PaymentForms.Add';
    ...
    var payload = {
      ...
      invoice_setting_id: p.invoice_setting_id,
      payment_gatway: p.payment_gatway != null ? p.payment_gatway : p.payment_gateway,
      payment_gateway: p.payment_gateway != null ? p.payment_gateway : p.payment_gatway,
      iteam_type: p.iteam_type || p.item_type || 'product',
      item_type: p.item_type || p.iteam_type || 'product',
      iteam_id: p.iteam_id != null ? p.iteam_id : (p.item_id != null ? p.item_id : p.payment_item_id)
    };
    if (p.document_type) payload.document_type = p.document_type;
    if (p.ask_to_customer != null) payload.ask_to_customer = p.ask_to_customer ? 1 : 0;
    if (p.change_value_with_customer != null) {
      payload.change_value_with_customer = p.change_value_with_customer ? 1 : 0;
    }
    if (p.required_fields != null) {
      payload.required_fields = typeof p.required_fields === 'string'
        ? p.required_fields
        : JSON.stringify(p.required_fields);
    }
    if (p.payment_type_data != null) {
      payload.payment_type_data = typeof p.payment_type_data === 'string'
        ? p.payment_type_data
        : JSON.stringify(p.payment_type_data);
    }
    // Dashboard flags — App API can rebuild payment_type_data from these when JSON omitted
    if (p.allow_installments != null) payload.allow_installments = p.allow_installments ? 1 : 0;
    if (p.max_installments != null) payload.max_installments = p.max_installments;
    if (p.allow_standing_order_hk != null) {
      payload.allow_standing_order_hk = p.allow_standing_order_hk ? 1 : 0;
    }
    if (p.max_installments_hk != null) payload.max_installments_hk = p.max_installments_hk;
    if (p.make_monthly_invoice != null) payload.make_monthly_invoice = p.make_monthly_invoice ? 1 : 0;
    if (p.allow_wild_card_product != null) {
      payload.allow_wild_card_product = p.allow_wild_card_product ? 1 : 0;
    }
    if (p.need_to_show_company_id != null) {
      payload.need_to_show_company_id = p.need_to_show_company_id ? 1 : 0;
    }
    if (p.regular_installments_document_type != null) {
      payload.regular_installments_document_type = p.regular_installments_document_type;
    }
    if (p.first_month_document_type != null) {
      payload.first_month_document_type = p.first_month_document_type;
    }
    if (p.every_month_document_type != null) {
      payload.every_month_document_type = p.every_month_document_type;
    }
    var raw = await getClient().request('PaymentForms.Add', payload);
    assertApiSuccess(raw, 'PaymentForms.Add', 'Payment form add failed');
    var id = raw.payment_form_id != null ? raw.payment_form_id : (raw.id != null ? raw.id : null);
    return { id: id, payment_form_id: id, message: raw.message || '', raw: raw };
  }

  /** PaymentForms.Update — update template (+ unpaid customer copies). */
  async function updatePaymentForm(params) {
    var p = params || {};
    var id = requireId(p.payment_form_id || p.id || p.data_id, 'payment_form_id');
    var payload = {
      payment_form_id: id,
      id: id,
      data_id: id
    };
    if (p.form_name != null || p.name != null) {
      payload.form_name = p.form_name != null ? p.form_name : p.name;
      payload.name = payload.form_name;
    }
    if (p.amount != null) payload.amount = String(p.amount);
    if (p.invoice_setting_id != null && p.invoice_setting_id !== '') {
      payload.invoice_setting_id = p.invoice_setting_id;
    }
    if (p.payment_gatway != null || p.payment_gateway != null) {
      payload.payment_gatway = p.payment_gatway != null ? p.payment_gatway : p.payment_gateway;
      payload.payment_gateway = payload.payment_gatway;
    }
    if (p.iteam_type != null || p.item_type != null) {
      payload.iteam_type = p.iteam_type || p.item_type;
      payload.item_type = payload.iteam_type;
    }
    if (p.iteam_id != null || p.item_id != null || p.payment_item_id != null) {
      payload.iteam_id = p.iteam_id != null ? p.iteam_id : (p.item_id != null ? p.item_id : p.payment_item_id);
      payload.item_id = payload.iteam_id;
      payload.payment_item_id = payload.iteam_id;
    }
    if (p.document_type != null) payload.document_type = p.document_type;
    if (p.ask_to_customer != null) payload.ask_to_customer = p.ask_to_customer ? 1 : 0;
    if (p.change_value_with_customer != null) {
      payload.change_value_with_customer = p.change_value_with_customer ? 1 : 0;
    }
    ...
    var raw = await getClient().request('PaymentForms.Update', payload);
    assertApiSuccess(raw, 'PaymentForms.Update', 'Payment form update failed');
    return {
      ok: true,
      payment_form_id: raw.payment_form_id != null ? raw.payment_form_id : id,
      message: raw.message || '',
      raw: raw
    };
  }

  /** PaymentForms.Delete — delete template. */
  async function deletePaymentForm(paymentFormId) {
    var id = requireId(paymentFormId, 'payment_form_id');
    var raw = await getClient().request('PaymentForms.Delete', {
      payment_form_id: id,
      id: id,
      data_id: id
    });
    assertApiSuccess(raw, 'PaymentForms.Delete', 'Payment form delete failed');
  ...
  async function countCustomerPaymentForms(customerId, extra) {
    var id = requireId(customerId, 'customer_id');
    var raw = await getClient().request('Customer.PaymentForms.Count', Object.assign({
      customer_id: id,
      cust_id: id
    }, extra || {}));
    return { customer_id: id, count: extractCount(raw), raw: raw };
  }

  async function addCustomerPaymentForm(customerId, paymentFormId) {
    var cid = requireId(customerId, 'customer_id');
    var fid = requireId(paymentFormId, 'payment_form_id');
    var raw = await getClient().request('Customer.PaymentForms.Add', {
      customer_id: cid,
      cust_id: cid,
      payment_form_id: fid
    });
    assertApiSuccess(raw, 'Customer.PaymentForms.Add', 'Assign payment form failed');
    var rowId = raw.payment_forms_paid_id != null ? raw.payment_forms_paid_id : raw.id;
    return { id: rowId, payment_forms_paid_id: rowId, message: raw.message || '', raw: raw };
  }

  async function updateCustomerPaymentForm(params) {
    var p = params || {};
    var cid = requireId(p.customer_id || p.cust_id, 'customer_id');
    var rowId = requireId(p.id || p.payment_forms_paid_id || p.data_id, 'payment_forms_paid_id');
    var payload = {
      customer_id: cid,
      cust_id: cid,
      id: rowId,
    ...
    var raw = await getClient().request('Customer.PaymentForms.Update', payload);
    assertApiSuccess(raw, 'Customer.PaymentForms.Update', 'Customer payment form update failed');
  ...
  async function listInvoiceSettings(extra) {
    var raw = await getClient().request('InvoiceSettings.List', Object.assign({
      limit: 25,
      length: 25,
      start: 0
    }, extra || {}));
    var rows = extractListRows(raw).map(function (r) {
      var id = r.id != null ? r.id : (r.invoice_setting_id != null ? r.invoice_setting_id : r.invoice_settings_id);
      return {
        id: id,
        invoice_setting_id: id,
        name: r.company_name || r.name || ('#' + id),
        company_name: r.company_name || r.name || '',
        company_type: r.company_type || '',
        is_default: !!(r.is_default || Number(r.default_settings) === 1),
        raw: r
      };
    });
    return { rows: rows, count: extractCount(raw) || rows.length, raw: raw };
  }

  /**
   * PaymentGateways.List — account gateways for payment_gatway (Settings → Payments).
   * Same pattern as InvoiceSettings.List / COMPANY dropdown.
   * Docs: https://eli.bull36.com/app/help/PaymentGateways.List
   */
  async function listPaymentGateways(extra) {
    var raw = await getClient().request('PaymentGateways.List', Object.assign({
      limit: 25,
      length: 25,
      start: 0
    }, extra || {}));
    var rows = extractListRows(raw).map(function (r) {
      var id = r.id != null ? r.id
        : (r.payment_gatway != null ? r.payment_gatway
          : (r.payment_detail_id != null ? r.payment_detail_id : r.payment_gateway_id));
      var method = r.payment_method || r.payment || r.method || r.provider || '';
      return {
        id: id,
        payment_gatway: id,
        ...
        provider: method,
        mode: r.mode,
        mode_label: r.mode_label || (Number(r.mode) === 1 ? 'Live' : (Number(r.mode) === 0 ? 'Test' : '')),
        is_default: !!(r.is_default || Number(r.default_payment) === 1),
        raw: r
      };
    });
    return { rows: rows, count: extractCount(raw) || rows.length, raw: raw };
  ...
  /** Products.List — linked items for iteam_id. */
  async function listProducts(extra) {
    var raw = await getClient().request('Products.List', Object.assign({
      limit: 25,
      length: 25,
      start: 0,
      active: 1
    ...
    var rows = extractListRows(raw).map(function (r) {
      var id = r.id != null ? r.id : r.product_id;
      return {
        id: id,
        name: r.product_name || r.name || ('#' + id),
  ...
  /**
   * Payment-form item picker (product / order / appointment / subscription).
   * Live dashboard uses GET /dashboard/setting2/ajaxPro?query=&data_id=&company_id=
   */
  async function listPaymentFormItems(itemType, extra) {
    var type = String(itemType || 'product').trim() || 'product';
    var opts = extra || {};
    var companyId = opts.company_id != null ? opts.company_id : (opts.invoice_setting_id || '');
    var rows = [];
    var rawParts = {};
    var seen = {};

    function pushRow(item, rowType) {
      ...
      rows.push({
        id: item.id,
        name: item.name || item.product_name || item.title || ('#' + item.id),
        price: item.price,
        type: rowType || type,
        raw: item
      });
    ...
    if (type === 'product') {
      try {
        var products = await listProducts({
          limit: opts.limit || 25,
          length: opts.limit || 25,
          start: 0,
          active: 1,
          search: opts.search || ''
        });
        rawParts.products = products.raw;
        (products.rows || []).forEach(function (r) { pushRow(r, 'product'); });
      } catch (e0) {
        rawParts.productsError = String(e0 && e0.message || e0);
      }
    }
    ...
    try {
      ...
      if (cred && cred.username && cred.password) {
        ...
        await global.fetch(DOMAIN + '/dashboard/login/check_login', {
          method: 'POST',
          credentials: 'include',
        ...
        // Cover names with single-letter queries (dashboard typeahead source).
        // Parallel GET is required — POST often returns [].
        var queries = (opts.query && String(opts.query).trim())
          ? [String(opts.query).trim()]
          : 'aeiouybcdfghjklmnpqrstvwxz0123456789'.split('');
        // Product already filled from Products.List — light ajaxPro pass only
        if (type === 'product' && rows.length) {
          queries = ['a', 'b', 'p', 't'];
        }
        var companyVariants = [companyId == null ? '' : String(companyId), ''];
        companyVariants = companyVariants.filter(function (v, i, arr) { return arr.indexOf(v) === i; });

        for (var ci = 0; ci < companyVariants.length; ci++) {
          var company = companyVariants[ci];
          var fetches = queries.map(function (q) {
            var url = DOMAIN + '/dashboard/setting2/ajaxPro'
              + '?query=' + encodeURIComponent(q)
              + '&data_id=' + encodeURIComponent(type)
              + '&company_id=' + encodeURIComponent(company);
            return global.fetch(url, { method: 'GET', credentials: 'include' })
              .then(function (res) { return res.text(); })
              .then(function (text) {
                var parsed = [];
                try { parsed = JSON.parse(text); } catch (eParse) { parsed = []; }
                return Array.isArray(parsed) ? parsed : [];
              })
              .catch(function () { return []; });
          });
          var batches = await Promise.all(fetches);
          batches.forEach(function (parsed) {
            (parsed || []).forEach(function (item) { pushRow(item, type); });
          });
          if (rows.length) break;
        }
        rawParts.ajaxProCount = rows.length;
      }
    } catch (e1) {
      rawParts.ajaxProError = String(e1 && e1.message || e1);
    }

    // Config seeds (real ids preferred) when account list is empty
    if (!rows.length) {
      var cfg = global.Biz1Config || {};
      var seeds = (cfg.seedPaymentFormItems && cfg.seedPaymentFormItems[type]) || [];
      seeds.forEach(function (item) { pushRow(item, type); });
      rawParts.usedSeeds = seeds.length;
    }

    // Last resort: for order/subscription with nothing configured, reuse products
    // so the select is never blank while account still has catalog items.
    if (!rows.length && (type === 'order' || type === 'subscription')) {
      try {
        var fallback = await listProducts({ limit: 25, length: 25, start: 0, active: 1 });
        (fallback.rows || []).forEach(function (r) {
          pushRow({
            id: r.id,
            name: (r.name || r.product_name || ('#' + r.id)) + ' (product)',
            price: r.raw && r.raw.price
          }, type);
        });
        rawParts.productFallback = true;
      } catch (e2) {
        rawParts.productFallbackError = String(e2 && e2.message || e2);
      }
    }
    ...
    return { rows: rows, count: rows.length, raw: rawParts, item_type: type };
  }
  ...
  function sortTsFromWhen(when) {
    if (!when) return 0;
    var n = Date.parse(when);
    if (!Number.isNaN(n)) return n;
    // he-IL / dd.mm.yyyy styles — best-effort
    var m = String(when).match(/(\d{1,2})\.(\d{1,2})\.(\d{4}).*?(\d{1,2}):(\d{2})(?::(\d{2}))?/);
    if (m) {
  ...
  function messageSortTs(row) {
    if (!row) return 0;
    var raw = row.raw || {};
    return sortTsFromWhen(
      row.time ||
      raw.time ||
      mongoDate(raw.inserted_date) ||
      mongoDate(raw.create_date) ||
      ''
    );
  }
  ...
  async function listSingleConversations(messengerMetaId, extra) {
    ...
    var rows = data.map(function (r) {
      ...
      return {
        ...
        email: r.email || '',
        time: r.time || chatWhen(r) || mongoDate(r.inserted_date),
    ...
    // API newest-first → oldest→newest for chat UI
    rows.sort(function (a, b) {
      return messageSortTs(a) - messageSortTs(b);
    });
  ...
  function classifyRealtimeEvent(event) {
    ...
    if (
      /payment[_\s-]?form|paymentforms|gvia|collection.?form/i.test(key) ||
      payload.payment_form_id != null ||
      payload.payment_forms_paid_id != null
    ) {
      return 'paymentForms';
    }
  ...
  function wireRealtimeHandlers(client) {
    ...
    client.realtime.on('biz1:ready', function (payload) {
      ...
      dispatchAppEvent('mineralbar:socket', {
        ...
        paymentForms: realtimeState.registered.filter(function (k) { return classifyRealtimeEvent({ key: k }) === 'paymentForms'; })
      });
    });
  ...
  global.Biz1App = {
    ...
    countPaymentForms: countPaymentForms,
    ...
    listCustomerPaymentForms: listCustomerPaymentForms,
    countCustomerPaymentForms: countCustomerPaymentForms,
    addCustomerPaymentForm: addCustomerPaymentForm,
    updateCustomerPaymentForm: updateCustomerPaymentForm,
    deleteCustomerPaymentForm: deleteCustomerPaymentForm,
    listInvoiceSettings: listInvoiceSettings,
    listPaymentGateways: listPaymentGateways,
    listProducts: listProducts,
    listPaymentFormItems: listPaymentFormItems,
