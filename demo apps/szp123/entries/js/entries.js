...
(function () {
  ...
  function updateView() {
    ...
    var isAllEntries = isData && !selectedCustomerId();

    navBackBtn.classList.toggle('hidden', isCustomers || isAllEntries);
    ...
    if (isData && tab) {
      navTitle.textContent = customerLabel
        ? prefix + (group ? EntriesAPI.groupName(group) + ' › ' : '') + EntriesAPI.tabName(tab)
        : EntriesUI.t('allCustomersTitle') + ' · ' + EntriesAPI.tabName(tab);
    } else if (group) {
      navTitle.textContent = prefix + EntriesAPI.groupName(group);
    } else if (customerLabel) {
      navTitle.textContent = customerLabel;
    } else {
      navTitle.textContent = EntriesUI.t('allCustomersTitle');
    }
    renderEntryPicker();
  }
  ...
  function uniqueTabs(tabs) {
    ...
    (tabs || []).forEach(function (tab) {
      var id = String(tab && (tab.id != null ? tab.id : tab.entry_id) || '');
      if (!id || seen[id]) return;
      seen[id] = true;
      out.push(tab);
    });
  ...
  function allEntryTabs() {
    return uniqueTabs((state.tabs || []).concat(state.list || []).concat(customerTabs()));
  }

  function firstTabId() {
    var tabs = allEntryTabs();
    if (!tabs.length) return '';
    return String(tabs[0].id != null ? tabs[0].id : tabs[0].entry_id);
  }
  ...
  function navPath(snap) {
    ...
    if (snap.navLevel === 'data' && snap.entryId) {
      if (snap.selectedCustomerId) {
        return '#/customer/' + encodeURIComponent(snap.selectedCustomerId) + '/entry/' + encodeURIComponent(snap.entryId);
      }
      return '#/entry/' + encodeURIComponent(snap.entryId);
    }
    if (snap.navLevel === 'entries' && snap.selectedGroupId && snap.selectedCustomerId) {
    ...
    if (snap.navLevel === 'customers') return '#/customers';
    return '#/entries';
  }
  ...
  function parseNavHash() {
    ...
    if (!parts.length || parts[0] === 'entries') {
      return {
        app: 'entries',
        navLevel: 'data',
        selectedCustomerId: '',
        entryId: parts[1] ? decodeURIComponent(parts[1]) : ''
      };
    }
  ...
  function applyNavSnapshot(snap) {
    ...
    try {
      ...
      if (snap.navLevel === 'data') {
        var cid = String(snap.selectedCustomerId || '');
        state.selectedCustomerId = cid;
        state.selectedCustomerLabel = cid
          ? String(snap.selectedCustomerLabel || state.selectedCustomerLabel || '')
          : '';
        state.selectedCustomerFolderId = cid
          ? String(snap.selectedCustomerFolderId || state.selectedCustomerFolderId || '')
          : '';
        state.selectedGroupId = cid ? String(snap.selectedGroupId || state.selectedGroupId || '') : '';
        if (!cid) {
          applyBasicTabs();
          state.customerDetail = null;
          openAllEntries(snap.entryId);
          return;
        }
        if (!state.customerDetail) loadCustomerDetail();
        if (snap.entryId) selectEntry(snap.entryId);
        else openGroups();
        return;
      }
      if (snap.navLevel === 'customers') {
  ...
  function openAllEntries(preferredEntryId) {
    state.selectedCustomerId = '';
    state.selectedCustomerLabel = '';
    state.selectedCustomerFolderId = '';
    state.customerDetail = null;
    ...
    if (searchInput) searchInput.value = '';
    applyBasicTabs();
    activateEntry(String(preferredEntryId || '').trim() || firstTabId(), 'replace');
  }
  ...
  function activateEntry(entryId, navMode) {
    state.entryId = String(entryId || '');
    state.navLevel = 'data';
    state.offset = 0;
    state.search = '';
    if (searchInput) searchInput.value = '';
    if (state.entryId) loadColumnPrefs(state.entryId);
    state.selectedIds = {};
    closeFieldPicker();
    updateView();
    renderFieldPicker();
    if (state.entryId) loadRows();
    else {
      state.rows = [];
      state.total = 0;
      renderRows();
    }
    syncNav(navMode || 'push');
  }

  function selectEntry(entryId) {
    activateEntry(entryId, 'push');
  }

  function navBack() {
    if (history.state && history.state.app === 'entries' && selectedCustomerId()) {
      history.back();
      return;
    }
    if (!selectedCustomerId()) {
  ...
  function columnLabel(col, index) {
    if (col.kind === 'customer') return EntriesUI.t('clientName');
  ...
  function renderBulkBar() {
    ...
    if (bulkDeleteBtn) {
      ...
      bulkDeleteBtn.setAttribute('aria-label', EntriesUI.t('deleteSelected') + (count ? ' (' + count + ')' : ''));
    }
  }
  ...
  function listEntriesAcrossCustomers(params) {
    return ensureCustomersLoaded().then(function (customers) {
      var list = (customers || []).slice(0, 25);
      if (!list.length) return Promise.reject(new Error(EntriesUI.t('entriesLoadFailed')));
      var merged = [];
      var chain = Promise.resolve();
      list.forEach(function (customer) {
        var cid = EntriesAPI.customerListId(customer);
        if (!cid) return;
        chain = chain.then(function () {
          return EntriesAPI.listEntries(Object.assign({}, params, {
            customer_id: cid,
            offset: 0,
            start: 0,
            limit: 25
          ...
          }).catch(function () { /* skip customer */ });
        });
      });
      return chain.then(function () {
        merged.sort(function (a, b) {
          return Number(EntriesAPI.rowId(b) || 0) - Number(EntriesAPI.rowId(a) || 0);
        });
        var start = Number(params.offset) || 0;
        var limit = Number(params.limit) || PAGE_SIZE;
        return { rows: merged.slice(start, start + limit), count: merged.length };
      });
    });
  }
  ...
  function loadRows() {
    ...
    return EntriesAPI.listEntries(params).then(applyListed).catch(function (err) {
      if (params.customer_id) {
        failList(err);
        return;
      }
      return listEntriesAcrossCustomers(params).then(applyListed).catch(function () {
        failList(err);
      });
    });
  }
  ...
  function syncDeleteModalCopy() {
    var count = (state.deleteRows && state.deleteRows.length) || (state.deleteRow ? 1 : 0);
    if (deleteModalTitle) {
      deleteModalTitle.textContent = count > 1
        ? EntriesUI.t('deleteSelectedTitle')
        : EntriesUI.t('deleteOneTitle');
    }
    if (deleteModalText) {
      deleteModalText.textContent = count > 1
        ? EntriesUI.t('deleteSelectedConfirm').replace('{n}', String(count))
        : EntriesUI.t('deleteOneText');
    }
  }

  function openDelete(row) {
    state.deleteRow = row || null;
    state.deleteRows = row ? [row] : [];
    syncDeleteModalCopy();
  ...
  function openBulkDelete() {
    ...
    var rows = ids.map(function (id) {
      return findRowById(id) || { id: id };
    });
    state.deleteRow = rows[0];
    state.deleteRows = rows;
    syncDeleteModalCopy();
    deleteModal.classList.remove('hidden');
  }
  ...
  window.addEventListener('popstate', function (e) {
    ...
    if (state.navLevel !== 'customers') {
      ...
      if (hashSnap) applyNavSnapshot(hashSnap);
      else openAllEntries();
    }
  });
  ...
  if (addEntryTopBtn) addEntryTopBtn.addEventListener('click', function () {
    openModal('add', null);
  });
  ...
  entryForm.addEventListener('submit', async function (e) {
    ...
    var pickedCustomerId = state.rowCustomerSelectedId
      ...
      : selectedCustomerId();
    if (state.modalMode === 'add' && !pickedCustomerId) {
      showFormError(EntriesUI.t('selectCustomer'));
      return;
    }
  ...
  confirmDeleteBtn.addEventListener('click', async function () {
    var rows = (state.deleteRows && state.deleteRows.length)
      ? state.deleteRows
      : (state.deleteRow ? [state.deleteRow] : []);
    if (!rows.length || !state.entryId) return;
    confirmDeleteBtn.disabled = true;
    confirmDeleteBtn.textContent = EntriesUI.t('deleting');
    try {
      for (var i = 0; i < rows.length; i += 1) {
        var id = EntriesAPI.rowId(rows[i]);
        if (!id) continue;
  ...
  refreshBasic()
    .then(function () {
      ...
      var hashSnap = parseNavHash();
      if (hashSnap && (hashSnap.selectedCustomerId || hashSnap.navLevel === 'customers')) {
        applyNavSnapshot(hashSnap);
      } else {
        openAllEntries(hashSnap && hashSnap.entryId);
      }
      syncNav('replace');
      wireRealtime();
    })
