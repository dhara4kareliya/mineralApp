(function () {
  ...
  function orderSearchText(row) {
    return [
      ...
      (function () {
        ...
        return Object.keys(bag).map(function (k) { return bag[k]; }).join(" ");
      })()
  ...
  function renderOrders() {
    ...
    App.$("ordersBody").innerHTML = state.orders.map(function (row) { return App.orderTableRow(row, true); }).join("");
    App.$("ordersCards").innerHTML = state.orders.map(function (row) { return App.orderCard(row, true); }).join("");
  ...
  async function loadOrders() {
    ...
    try {
      ...
      var rows = list.rows || [];
      if (search) {
        var matched = rows.filter(function (row) { return matchesOrderSearch(row, search); });
        if (!matched.length) {
          ...
          matched = (fallback.rows || []).filter(function (row) { return matchesOrderSearch(row, search); });
        }
  ...
  async function boot() {
    ...
    App.$("createOrderBtn").addEventListener("click", function () {
      App.openOrderModal({ catalogId: state.selectedCatalogId });
    });
    ["filterStatus", "filterPaid", "filterFrom", "filterTo"].forEach(function (id) {
      App.$(id).addEventListener("change", function () { state.orderStart = 0; loadOrders(); });
    });
    function runSearch() {
      state.orderStart = 0;
      loadOrders();
    }
    App.$("filterSearch").addEventListener("input", function () {
      clearTimeout(state.searchTimer);
      state.searchTimer = setTimeout(runSearch, 250);
    });
    App.$("filterSearch").addEventListener("keydown", function (e) {
      if (e.key === "Enter") {
        e.preventDefault();
        clearTimeout(state.searchTimer);
        runSearch();
    ...
    App.$("ordersPrev").addEventListener("click", function () {
      state.orderStart = Math.max(0, state.orderStart - App.PAGE_SIZE);
      loadOrders();
    });
    App.$("ordersNext").addEventListener("click", function () {
