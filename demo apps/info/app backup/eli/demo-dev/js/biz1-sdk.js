 ...
 */
(function _biz1SdkSub1(root, factory) {
  if (typeof module === 'object' && module.exports) {
    module.exports = factory();
  } else {
    root.Biz1SDK = factory();
  }
})(typeof globalThis !== 'undefined' ? globalThis : window, function _biz1SdkSub2() {
  'use strict';
  ...
  function createMemoryStorage() {
    var data = {};
    return {
      getItem: function _biz1SdkSub3(key) { return Object.prototype.hasOwnProperty.call(data, key) ? data[key] : null; },
      setItem: function _biz1SdkSub4(key, value) { data[key] = String(value); },
      removeItem: function _biz1SdkSub5(key) { delete data[key]; }
    };
  }
  ...
  function appendBody(body, key, value) {
    ...
    if (Array.isArray(value)) {
      value.forEach(function _biz1SdkSub6(item) { body.append(key, normalizeDateInput(key, item)); });
      return;
    }
  ...
  function convertFormData(data) {
    ...
    data.forEach(function _biz1SdkSub7(value, key) {
  ...
  function convertUrlSearchParams(data) {
    ...
    data.forEach(function _biz1SdkSub8(value, key) {
      body.append(key, normalizeDateInput(key, value));
    });
    return body;
  }
  ...
  function toBody(data) {
    ...
    var body = new URLSearchParams();
    Object.keys(data).forEach(function _biz1SdkSub9(key) { appendBody(body, key, data[key]); });
    return body;
  }
  ...
  function capListInput(input) {
    ...
    ['limit', 'length', 'per_page'].forEach(function _biz1SdkSub10(key) {
  ...
  function routeProxy(client, prefix) {
    return new Proxy(function _biz1SdkSub11() {}, {
      get: function _biz1SdkSub12(_target, prop) {
        if (prop === 'then') return undefined;
        var next = prefix ? prefix + '.' + String(prop) : String(prop);
        return routeProxy(client, next);
      },
      apply: function _biz1SdkSub13(_target, _thisArg, args) {
  ...
  Biz1RealtimeClient.prototype.deviceId = function _biz1SdkSub14() {
  ...
  Biz1RealtimeClient.prototype.lastEventId = function _biz1SdkSub15() {
    return Number(this.storage.getItem(LAST_EVENT_ID_KEY) || 0);
  };

  Biz1RealtimeClient.prototype.setLastEventId = function _biz1SdkSub16(eventId) {
    if (!eventId || Number(eventId) <= this.lastEventId()) return false;
    this.storage.setItem(LAST_EVENT_ID_KEY, String(Number(eventId)));
    return true;
  };

  Biz1RealtimeClient.prototype.resolveIo = function _biz1SdkSub17() {
  ...
  Biz1RealtimeClient.prototype.connect = function _biz1SdkSub18(options) {
    options = options || {};
    ...
    this.socket.on('biz1:event', function _biz1SdkSub19(event) {
    ...
    this.socket.on('biz1:ready', function _biz1SdkSub20(payload) {
      self.emitLocal('biz1:ready', payload);
    });

    this.socket.on('rooms:refresh', function _biz1SdkSub21(event) {
  ...
  Biz1RealtimeClient.prototype.on = function _biz1SdkSub22(eventKey, handler) {
    if (!this.handlers[eventKey]) this.handlers[eventKey] = [];
    this.handlers[eventKey].push(handler);
    var self = this;
    return function off() {
      self.handlers[eventKey] = (self.handlers[eventKey] || []).filter(function _biz1SdkSub23(fn) { return fn !== handler; });
    };
  };

  Biz1RealtimeClient.prototype.emitLocal = function _biz1SdkSub24(eventKey, payload) {
    (this.handlers[eventKey] || []).slice().forEach(function _biz1SdkSub25(handler) {
      handler(payload);
    });
  };

  Biz1RealtimeClient.prototype.disconnect = function _biz1SdkSub26() {
    if (this.socket) this.socket.disconnect();
    this.socket = null;
  };
  ...
  Biz1Client.prototype.installAliases = function _biz1SdkSub27() {
    var self = this;
    this.account = {
      basic: function _biz1SdkSub28() { return self.request('User.Basic'); }
    };
    this.customers = {
      list: function _biz1SdkSub29(filters) { return self.list('Customer.List', filters); },
      count: function _biz1SdkSub30(filters) { return self.count('Customer.Count', filters); },
      get: function _biz1SdkSub31(customerId, extra) { return self.request('Customer.Get', Object.assign({ customer_id: customerId }, extra || {})); },
      add: function _biz1SdkSub32(data) { return self.request('Customer.Add', data); },
      update: function _biz1SdkSub33(customerId, data) { return self.request('Customer.Edit', Object.assign({ customer_id: customerId }, data || {})); },
      remove: function _biz1SdkSub34(customerId) { return self.request('Customer.Delete', { customer_id: customerId }); }
    };
    [
      ['Mission', 'missions'],
      ['Ticket', 'tickets'],
      ...
      ['Entries', 'entries'],
      ['Expenses', 'expenses']
    ].forEach(function _biz1SdkSub35(pair) {
      var category = pair[0];
      var group = pair[1];
      self[group] = {
        list: function _biz1SdkSub36(filters) { return self.list(category + '.List', filters); },
        count: function _biz1SdkSub37(filters) { return self.count(category + '.Count', filters); },
        get: function _biz1SdkSub38(id, extra) { return self.request(category + '.Get', Object.assign({ id: id }, extra || {})); },
        add: function _biz1SdkSub39(data) { return self.request(category + '.Add', data); },
        update: function _biz1SdkSub40(id, data) { return self.request(category + '.Update', Object.assign({ id: id }, data || {})); },
        remove: function _biz1SdkSub41(id) { return self.request(category + '.Delete', { id: id }); }
      };
    });
  };

  Biz1Client.prototype.getToken = function _biz1SdkSub42() {
    return this.storage.getItem(TOKEN_KEY) || '';
  };

  Biz1Client.prototype.setToken = function _biz1SdkSub43(token) {
    if (token) this.storage.setItem(TOKEN_KEY, token);
    else this.storage.removeItem(TOKEN_KEY);
  };

  Biz1Client.prototype.login = async function _biz1SdkSub44(credentials) {
  ...
  Biz1Client.prototype.loginWithOtp = async function _biz1SdkSub45(credentials) {
    credentials = credentials || {};
    var data = await this.request('LoginWithOtp', {
      username: credentials.username || credentials.email || credentials.user || credentials.id || '',
      otp: credentials.otp || ''
    }, { public: true });
    if (data && data.token) this.setToken(data.token);
    return data;
  ...
  Biz1Client.prototype.logout = function _biz1SdkSub46() {
    this.setToken('');
    this.realtime.disconnect();
  };

  Biz1Client.prototype.request = async function _biz1SdkSub47(route, data, options) {
    options = options || {};
  ...
  Biz1Client.prototype.list = async function _biz1SdkSub48(route, filters) {
  ...
  Biz1Client.prototype.count = async function _biz1SdkSub49(route, filters) {
    var raw = await this.request(route, capListInput(filters || {}));
  ...
  return {
    ...
    createClient: function _biz1SdkSub50(options) { return new Biz1Client(options); }
  };
});
